...
Source: `roszczenia.csv` (gitignored — contains KW numbers and owner
names). Schema lives in migrations `003_roszczenia.py`,
`004_roszczenia_rename_col.py`, `005_roszczenia_kw_entities.py`.
...
# CSV lives in backend/ on the host (gitignored). Adjust the filename if
# you renamed it — the script matches "backend/roszczenia*.csv" in
# .gitignore so any roszczenia-*.csv variant is fine.
docker compose -f docker-compose.prod.yml exec backend \
  python scripts/ingest_roszczenia_csv.py /app/backend/roszczenia.csv
```

If the CSV isn't on the host yet, copy it from your laptop first:

```bash
scp "roszczenia (1).csv" \
    deploy@45.137.213.188:/home/deploy/dzialkowizja/backend/roszczenia.csv
```
